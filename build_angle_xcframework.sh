...
### PRINT ANGLE VERSION ###
# Read version exactly as compiled into the binaries:
#   - major/minor:  src/common/angle_version.h (checked in)
#   - revision:     ANGLE_COMMIT_POSITION from angle_commit.h (generated by commit_id.py)
#   - hash:         ANGLE_COMMIT_HASH from angle_commit.h
VERSION_HEADER="src/common/angle_version.h"
COMMIT_HEADER="${OUT_DIR}/ios-arm64/gen/angle/angle_commit.h"

MAJOR=$(sed -n 's/^#define ANGLE_MAJOR_VERSION //p' "${VERSION_HEADER}")
MINOR=$(sed -n 's/^#define ANGLE_MINOR_VERSION //p' "${VERSION_HEADER}")
REV=$(sed -n 's/^#define ANGLE_COMMIT_POSITION //p' "${COMMIT_HEADER}" 2>/dev/null)
HASH=$(sed -n 's/^#define ANGLE_COMMIT_HASH "\(.*\)"$/\1/p' "${COMMIT_HEADER}" 2>/dev/null)

# Fallback to git when generated header is unavailable
if [[ -z "${REV}" || "${REV}" == "0" ]]; then
  REV=$(git rev-list HEAD --count)
fi
if [[ -z "${HASH}" || "${HASH}" == "unknown hash" ]]; then
  HASH=$(git rev-parse --short=12 HEAD)
fi

echo ""
echo "ANGLE ${MAJOR}.${MINOR}.${REV} git hash: ${HASH}"
